...
def run_main_estimation(sc_inertia, channels, sensors: RealData, mag_i_on_obc, max_samples=None,
                        online_mag_calibration=False, pred_step_sec=0, sigma_bias=1e-4, sigma_omega=np.deg2rad(0.2),
    ...
    # delta theta, delta bias
    # P = 1 * np.diag([
    #                 1e-1, 1e-1, 1e-1,  # ~57 deg attitude uncertainty
    #                 1, 1, 1   # ~0.6 deg/s bias uncertainty
    #             ])
    P = np.diag([
        (50 * np.deg2rad(1)) ** 2,  # Attitude error ~50 deg
        (50 * np.deg2rad(1)) ** 2,
        (50 * np.deg2rad(1)) ** 2,
        (0.35) ** 2,  # Bias error ~20 deg/s (0.35 rad/s)
        (0.35) ** 2,
        (0.35) ** 2
    ]) * 10
    # ekf_model = MEKF(sc_inertia, P=P, Q=np.zeros((6, 6)), R=np.zeros((3, 3)))
    ekf_model = MEKF_OBC(P)
    ekf_model.sigma_u_bias = sigma_bias  # gyro noise standard deviation [rad/s]
    ekf_model.sigma_v_omega = sigma_omega  # gyro random walk standard deviation [rad/s*s^0.5]
    ...
    for ch_idx, t_jd in rng:
        ...
        # if verbose:
        #     rng_pred = tqdm(rng_pred, total=int(pred_step_sec / 0.1),
        #                     desc=f"Prediction calculation - {omega_b_pred}",
        #                     leave=False)
        for _ in rng_pred:
